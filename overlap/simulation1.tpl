...
DATA_SECTION
  ...
  init_matrix recruitPar(1,numPop,1,6)      // #column for(fecundity alpha, density depend. beta, std for recruit.) 
  ...
  init_int effSampleSize                    // effective sample size 200 or 800
  ...
  3darray real_N(1,numSim,1,numPop,1,numYear)                                //area specific abundance
  3darray assess_N(1,numSim,1,numPop,1,numYear)                                //area specific abundance *weight
 ...
 LOC_CALCS
   ...
   recruAlpha = column(recruitPar,1); // logistic recruit parameter
   recruBeta  = column(recruitPar,2);
   initAbund  = column(recruitPar,3);
   MSY        = column(recruitPar,4);
   EY         = column(recruitPar,5);
   recruStd   = column(recruitPar,6);
...
PRELIMINARY_CALCS_SECTION 
  ...
  Recruit.initialize(); F.initialize(); 
  
  pooledBegYr.initialize();yieldTolarea.initialize();pooledTAC.initialize();tacAge_est.initialize(); 
  catchTolarea.initialize(); areaCPE.initialize();TotalCPE.initialize();CPEpar.initialize();
  real_N.initialize(); assess_N.initialize(); N_mre.initialize(); N_mare.initialize();mixBegYr.initialize();
  ...
  for(int i=1;i<=numSim;i++){          // **simulation loop  i
    ...
    //*********************************   first year data simulation 
      ...
      for(int k=1;k<=numPop;k++) effort(i,j,k)=1./q*fishMort(k)*mfexp((sdEffort(k)*randn(rnd))-(0.5*square(sdEffort(k))));      // fishing effort:  E= F*exp(sd-0.5*sigma^2) /q
      ...
      if (model==2) {
       ...
       for(int m=1;m<=numAge;m++){                  // begin  m
        for(int k=1;k<=numPop;k++){                 // begin  k
         ...
         pooledBegYr(i,j,m)+= popBegYr(i,k,j,m); // cal pooledmixBegYr
        }                                           // end k      
    ...
    // **************************************************************after first year &  before starting assessment year data simulation
      if(j > 1 && j<=assessYr){ 
        for(int n=1;n<=numPop;n++){    
          ...
          tmpPop(1)=recruAlpha(n)*SSB(i,j-1,n)*mfexp(-recruBeta(n)*SSB(i,j-1,n))*mfexp(recruStd(n)*randn(rnd)); 
        ...
        for(int m=1;m<=numAge;m++){ 
          for(int k=1;k<=numPop;k++){  
           ...
           pooledBegYr(i,j,m)+= popBegYr(i,k,j,m);
          }         // end  k
          pooledYield(i,j,m)=pooledcatch(i,j,m)*weight(m);
         }            // end  m
        for(int k=1;k<=numPop;k++)  pooledeffort(i,j)+= effort(i,j,k);
...
// model=3 :: meta population assessment model
// Begins in year 22
// ***********************************************
  ...
  if(j > assessYr){                        // use new F for year after assessment started
        ...
        for(int n=1;n<=numPop;n++){ 
          tmpPop(n,1)=recruAlpha(n)*SSB(i,j-1,n)*mfexp(-recruBeta(n)*SSB(i,j-1,n))*mfexp(recruStd(n)*randn(rnd)); // using SSB from previous year
        ...
        if ((model==1)|(model==3)){ // begin model 1 : independent population model 1
         for(int k=1;k<=numPop;k++){ 
        ...
        for(int k=1;k<=numPop;k++)  effort(i,j,k)=1./q*newF(k)*mfexp((sdEffort(k)*randn(rnd))-(0.5*square(sdEffort(k))));      // fishing effort:  E= F*exp(sd-0.5*sigma^2) /q
      ...
      if(j == assessOK){ //start on assessYr, then every 3 yrs after 
       ...
       //cal true mean Rec AND initial Abun, used as starting value for assessment model, to availed hessian warning.
         ...
         cout<<"Rec Mean"<<RecPop<<endl;
          ...
          readin>>assess_N(i,k,j); readin>>catch_mre(i,k,j); 
          readin>>catch_mare(i,k,j); readin>>assess_ssb(i,k,j);
          readin.close();
          ...
          for(int m=assessFAge;m<=assessLAge;m++)  real_N(i,k,j)+=popBegYr(i,k,j-1,m); // number of fish -real
          N_mre(i,k,j)=(assess_N(i,k,j)- real_N(i,k,j))/(real_N(i,k,j)+EPS);       //mre for aboundance- number of fish
          N_mare(i,k,j)=abs(assess_N(i,k,j)- real_N(i,k,j))/(real_N(i,k,j)+EPS);   //mare for aboundance          
       ...
       if(model==3)  {
         ...
         //exit(11);
             ...
             //output data file for assessment
          ofstream report("sim4assessment2.dat");
          ...
          if(forLinux==0) int tmp12=system("sim4assessment2 -nox > NUL"); //run assessment program
          else int tmp12=system("./sim4assessment2 -nox > /dev/null");    //run assessment program
          
          //read in TAC from assessment report file
          ifstream readin("sim4assessment2.rep");  
          readin>>readInTAC(1);
          readin>>readInTAC(2);
          readin>>readInTAC(3);
          readin>>readInTAC(4);
          ...
          readin>>grad;
          //Grad(i,k,j)= double(grad);
          readin>>assess_N(i,1,j); 
          readin>>assess_N(i,2,j); 
          readin>>assess_N(i,3,j); 
          readin>>assess_N(i,4,j); 
          ...
          //if got hessian warning
           ifstream ifs("sim4assessment2.std");
           if(ifs) {
            ifs.close();
            if(forLinux==0) int tmp14=system("del sim4assessment2.std");
            else int tmp14=system("rm -f sim4assessment2.std");
           }
           else { 
            cout<<"******************************** Warning: The function maximizer failed !" <<endl;
            //exit(5);
          ...
          if(forLinux==0) int tmp13=system("del sim4assessment2.rep");
          else int tmp13=system("rm -f sim4assessment2.rep");
       ...
       //********************pooled population assessment model begins here*****************************
         ...
         dvariable RecPop; 
          //cout<<endl<<"#==== N  (yr x age) "<<k <<" ===="<<endl; //output catch at age     
          if(useAllYr==1){
            IntPop=mean(pooledBegYr(i,1)(assessFAge+1,assessLAge));
            for(int yr=1;yr<=j-1;yr++) { //use all previous yr data
             dvector TmpPop(1,j-1);
             TmpPop(yr)=pooledBegYr(i,yr,assessFAge);
             if(yr==j-1)  RecPop=mean(TmpPop);
            }	   
          }else{                        //output fixed num yr data
            int startYr=j-nYrAssess;
            IntPop=mean(pooledBegYr(i,startYr)(assessFAge+1,assessLAge));
            for(int yr=startYr;yr<=j-1;yr++){
              dvector TmpPop(startYr,j-1);
              TmpPop(yr)=pooledBegYr(i,yr,assessFAge);
              if(yr==j-1) RecPop=mean(TmpPop);
         ...
         cout<<"Rec Mean"<<RecPop<<endl;
          ...
          double pooledgrad;
          readin>>pooledgrad;
          readin>>pooled_assess_ssb(i, j); readin>>pooledcatch_mre(i, j); readin>>pooledcatch_mare(i, j);
          //readin>>tacAge_est;
          //readin>>Ftac_est;
          //readin>>rc_s_est;
          //for(int q=1; q<=20; q++) {
         ...
         if(ifs) {                                                                                                              
          ...
          for(int m=assessFAge;m<=assessLAge;m++){ //cross assessment age
            pooled_real_ssb(i,j)+=pooledBegYr(i,j-1,m)*weight(m)*propFemale*maturity(m); //same last yr as in estimation model? j or j-1
          }
          pooled_ssb_mre(i,j)=(pooled_assess_ssb(i,j)- pooled_real_ssb(i,j))/(pooled_real_ssb(i,j)+EPS);
          pooled_ssb_mare(i,j)=abs(pooled_assess_ssb(i,j)- pooled_real_ssb(i,j))/(pooled_real_ssb(i,j)+EPS);                                                     
         } //end if model==2
